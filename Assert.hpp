 ...
 *
 * This file is a copy of the Assert.hpp in CMacIonize (Vandenbroucke & Wood,
 * 2018).
 *
...
#define assert_condition(condition)                                            \
  if (!(condition)) {                                                          \
    nnr_error("Assertion failed (%s)!", #condition);                           \
  }
...
#define assert_values_equal_tol(a, b, tol)                                     \
  if (std::abs((a) - (b)) > tol &&                                             \
    ...
    nnr_error("Assertion failed: %s (%g) != %s (%g)", #a, a, #b, b);           \
  }
