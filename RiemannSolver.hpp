 ...
 * The version in this file corresponds to the exact Riemann solver in
 * CMacIonize (Vandenbroucke & Wood, 2018).
 *
...
public:
  /**
   * @brief Possible solution structures of the Riemann problem.
   */
  enum RiemannSolutionStructure {
    /*! @brief Left and right shock wave. */
    SHOCK_SHOCK = 0,
    /*! @brief Left shock wave, right rarefaction wave. */
    SHOCK_RAREFACTION,
    /*! @brief Left rarefaction wave, right shock wave. */
    RAREFACTION_SHOCK,
    /*! @brief Left and right rarefaction wave. */
    RAREFACTION_RAREFACTION,
    /*! @brief Vacuum solution. */
    VACUUM
  };
...
private:
  ...
  inline double solve_brent(double PL, double AL, double BL, double PLinv,
    ...
    if (fa * fb > 0.) {
      nnr_error("Equal sign function values provided to solve_brent (%g %g)!",
                fa, fb);
    }
   ...
   * @param Psol Pressure solution.
   * @param dxdt Point in velocity space where we want to sample the solution.
   * @return Structure of the solution.
   */
  inline RiemannSolutionStructure
  sample_right_vacuum(double rhoL, double uL, double PL, double aL,
                      double &rhosol, double &usol, double &Psol,
                      double dxdt = 0.) const {
   ...
   * @return Structure of the solution.
   */
  inline RiemannSolutionStructure sample_left_vacuum(double rhoR, double uR,
                                                     double PR, double aR,
                                                     double &rhosol,
                                                     double &usol, double &Psol,
                                                     double dxdt = 0.) const {
   ...
   */
  inline RiemannSolutionStructure
  solve_vacuum(const double rhoL, const double uL, const double PL,
               const double aL, const bool vacuumL, const double rhoR,
               const double uR, const double PR, const double aR,
               const bool vacuumR, double &rhosol, double &usol, double &Psol,
...
public:
   ...
   */
  RiemannSolver(const double gamma)
    ...
    if (gamma < 1.) {
      nnr_error("The adiabatic index needs to be 1 or larger!")
    }
  }
   ...
   */
  inline RiemannSolutionStructure
  solve(const double rhoL, const double uL, const double PL, const double rhoR,
        const double uR, const double PR, double &rhosol, double &usol,
        double &Psol, const double dxdt = 0.) const {

    // check input
    nnr_assert(rhoL >= 0.);
    nnr_assert(rhoL == rhoL);
    nnr_assert(uL == uL);
    nnr_assert(PL >= 0.);
    nnr_assert(PL == PL);
    nnr_assert(rhoR >= 0.);
    nnr_assert(rhoR == rhoR);
    nnr_assert(uR == uR);
    nnr_assert(PR >= 0.);
    nnr_assert(PR == PR);
